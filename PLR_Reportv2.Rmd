...
#write_csv(tab_cascade,"tab_cascade.csv")
...
tab_cascade_gather <- gather(tab_cascade_spread,"outcomes","val",1:17)
...
## PLR Cascade From Oct 2016 - to July 2017
```{r}
start_ltfu <- date("2016-10-01")
end_ltfu <- date("2017-07-31")

LTFU_2_FY17 <- filter(LTFU_2,datesuivieffectue_clean >= start_ltfu, datesuivieffectue_clean < end_ltfu)
tab_fy17 <- table(LTFU_2_FY17$patient_outcome)
tab_cascade_fy17 <- as_data_frame(tab_fy17)

names(tab_cascade_fy17) <- c("outcomes","nb")
...
## calulate outcome percent
tab_cascade_fy17 <- f_cascade(tab_cascade_fy17)

tab_cascade_spread_p_fy17 <- spread(tab_cascade_fy17[,c(1,3)],outcomes,percent)
tab_cascade_spread_n_fy17 <- spread(tab_cascade_fy17[,c(1,2)],outcomes,nb)

tab_cascade_spread_fy17 <- spread(tab_cascade_fy17[,c(1,2)],outcomes,nb)
tab_cascade_spread_fy17$no_outcome_total <-tab_cascade_spread_fy17$no_outcome+tab_cascade_spread_fy17$PatientIntrouvable

tab_cascade_spread_fy17$no_outcome_percent <-tab_cascade_spread_fy17$no_outcome_total /tab_cascade_spread_fy17$`Tot. Contacted`

tab_cascade_spread_fy17$KnownOutcomes_percent <- tab_cascade_spread_fy17$`Known Outcomes`/tab_cascade_spread_fy17$`Tot. Contacted`
tab_cascade_spread_fy17$BackInClinic_percent <- tab_cascade_spread_fy17$PatientRetourneALaClinique/tab_cascade_spread_fy17$`Known Outcomes`

tab_cascade_spread_fy17$RefuseTx_percent <- tab_cascade_spread_fy17$PatientRefuse/tab_cascade_spread_fy17$`Known Outcomes`
tab_cascade_spread_fy17$SilentTransfer_percent <- tab_cascade_spread_fy17$PatientSuiviAilleurs/tab_cascade_spread_fy17$`Known Outcomes`
tab_cascade_spread_fy17$Dead_percent <- tab_cascade_spread_fy17$PatientDecede/tab_cascade_spread_fy17$`Known Outcomes`
tab_cascade_spread_fy17$active_percent <- tab_cascade_spread_fy17$PatientActif/tab_cascade_spread_fy17$PatientRetourneALaClinique

## flowchart


    par(mar = c(1, 1, 1, 1))
    openplotmat(main ="LTFU Flowchart of outcomes Oct 2016- July 2017")
    elpos <- coordinates (c(1, 2, 4, 2))
    fromto <- matrix(ncol = 2,byrow = T,data = c(1,2,1,3,3,4,3,5,3,6,3,7,7,9))
    nr <- nrow(fromto)
    arrpos <- matrix(ncol = 2, nrow = nr)
    for(i in 1:nr)
        arrpos[i,] <- straightarrow(to = elpos[fromto[i,2],],
                                    ...
                                    arr.length = 0.5)
    textround(elpos[1,], 0.15,0.06,
    lab = c("Total Contacted",tab_cascade_spread_fy17$`Tot. Contacted`), cex = 0.8)
    textround(elpos[2,], 0.15,0.07,
    lab = c("No Outcomes found",paste(tab_cascade_spread_fy17$no_outcome_total,
                                 "(",round(tab_cascade_spread_fy17$no_outcome_percent*100,1),"%)")), cex = 0.8)
   
    textround(elpos[3,], 0.15,0.07, 
    lab = c("Outcomes found",paste(tab_cascade_spread_n_fy17$`Known Outcomes`,
                                   "(",round(tab_cascade_spread_fy17$KnownOutcomes_percent*100,1),"%)")),  cex = 0.8)
    
    textround(elpos[4,], 0.05,0.07,
    lab = c("Dead",paste(tab_cascade_spread_n_fy17$PatientDecede,
                        "(",round(tab_cascade_spread_fy17$Dead_percent*100,1),"%)")),  cex = 0.8)
    
    textround(elpos[5,], 0.05,0.07, 
    lab = c("Silent transfer",paste(tab_cascade_spread_n_fy17$PatientSuiviAilleurs,
                          "(",round(tab_cascade_spread_fy17$SilentTransfer_percent*100,1),"%)")), cex = 0.8)
    
    textround(elpos[6,], 0.05,0.07, 
    lab = c("Refuse Treatment",paste(tab_cascade_spread_n_fy17$PatientRefuse,
                            "(",round(tab_cascade_spread_fy17$RefuseTx_percent*100,1),"%)")),  cex = 0.8)
    
    textround(elpos[7,], 0.05,0.07,
    lab = c("Back in clinic",paste(tab_cascade_spread_n_fy17$PatientRetourneALaClinique,
                                   "(",round(tab_cascade_spread_fy17$BackInClinic_percent*100,1),"%)")),cex = 0.8)
    
    textround(elpos[9,], 0.05,0.07,
    lab = c("active on treatment",paste(tab_cascade_spread_n_fy17$PatientActif,
                                        "(",round(tab_cascade_spread_fy17$active_percent*100,1),"%)")),  cex = 0.8)
